...
cod int auto_increment,
produtora varchar(40) not null unique,
...
-- Mostrando conteúdo da Tabela
select * from generos;

-- Alimentando a Tabela Produtoras
describe produtoras;

-- Outra opção inserindo valores apenas de uma coluna especifica. (dessa forma como o cód e auto incremento, não preciso informar essa coluna).
insert into produtoras (produtora, pais) values
('Microsoft', 'EUA'),
('Tencent', 'China'),
('Nintendo', 'Japão'),
('Sony', 'Japão'),
...
-- Alimentando a Tabela Jogos
describe jogos;

-- Nesta inserção, mesmo o campo cod sendo auto incremento, vou informar os valores dentro da coluna.
...
-- Outra forma de usar o select, indicando o nome do BD e o nome da Tabela
select * from bd_games.jogos;

-- Alimentando a Tabela Usuários
describe usuarios;

insert into usuarios (login, nome, senha, tipo) values
('admin', 'Darlan Araujo', '$2y$10$UpMQCcir.v649HrdLvUXiOC/ftU7xWhSxm8QhX.VzSe9LZHhvW/Ty', 'admin'),
('davi', 'Davi Araujo', '$2y$10$w7on7cjLKNtmJUGkiHIXoOQAwTJzkgxXqLmjtfDUkCXmQK0784.IS', 'editor');
